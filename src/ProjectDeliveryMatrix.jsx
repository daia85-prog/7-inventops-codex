...
import {
  ArrowRight, Buildings, CheckCircle, Clock, Envelope, FileText,
  LinkSimple, User, Warning
...
export function ProjectDeliveryMatrix({project,onUpdate,notify}){
  ...
  const updateSelected=(changes,message)=>{
    persist(deliveries.map(item=>item.id===selected.id?{...item,...changes,updated:"Agora"}:item),message);
  };
  const chooseFilter=option=>{
    setFilter(option);
    const candidates=option==="Todas"?deliveries:deliveries.filter(item=>option==="Atenção"?["Em risco","Aguardando"].includes(item.status):item.status===option);
  ...
  const registerEvidence=()=>{
    ...
    updateSelected({evidence:evidence.trim(),status:selected.status==="Planejada"?"Em validação":selected.status},"Evidência registrada e vinculada à entrega.");
  };
  const conclude=()=>{
  ...
  return <div className="pdm">
    <section className="pdm-hero">
      <div><small>CONTROLE MATRICIAL DO PROJETO</small><h3>Entregas por área, sem perder o trabalho paralelo</h3><p>Cada área responde por uma entrega verificável. Uma atividade só bloqueia outra quando existe dependência técnica real.</p></div>
      <span><Buildings/><b>{deliveries.length}</b><small>áreas conectadas</small></span>
    </section>

    <section className="pdm-summary">
      <article><small>PROGRESSO MÉDIO</small><b>{summary.average}%</b><i><em style={{width:`${summary.average}%`}}/></i></article>
      <article><small>CONCLUÍDAS</small><b>{summary.completed}</b><span className="success"><CheckCircle/>com evidência</span></article>
      <article><small>EM EXECUÇÃO</small><b>{summary.running}</b><span><Clock/>inclui paralelo</span></article>
      <article className={summary.attention?"attention":""}><small>EXIGEM ATENÇÃO</small><b>{summary.attention}</b><span><Warning/>risco ou espera</span></article>
    </section>

    <div className="pdm-toolbar">
      <div>{["Todas","Em andamento","Em paralelo","Atenção","Planejada"].map(option=><button key={option} className={filter===option?"active":""} onClick={()=>chooseFilter(option)}>{option}</button>)}</div>
    ...
    <div className="pdm-workspace">
      <section className="pdm-grid" aria-label="Entregas das áreas">
        {filtered.map(item=><button className={`pdm-card ${tones[item.status]} ${selected.id===item.id?"selected":""}`} key={item.id} onClick={()=>setSelectedId(item.id)}>
          <header><span>{item.area}</span><em>{item.status}</em></header>
          <h4>{item.delivery}</h4>
          <p><User/>{item.owner}</p>
          <div><i><em style={{width:`${item.progress}%`}}/></i><b>{item.progress}%</b></div>
          <footer><small>Prazo {item.due}</small><small>{item.updated}</small></footer>
        </button>)}
      </section>

      <aside className="pdm-detail">
        <header><span>{selected.area}</span><div><small>{selected.department}</small><h3>{selected.delivery}</h3></div><em className={tones[selected.status]}>{selected.status}</em></header>
        <div className="pdm-detail-progress"><span><b>{selected.progress}%</b><small>progresso comprovado</small></span><i><em style={{width:`${selected.progress}%`}}/></i></div>
        <dl>
          <div><dt><User/>Responsável</dt><dd><b>{selected.owner}</b><small>{selected.email}</small></dd></div>
          <div><dt><Clock/>Prazo da entrega</dt><dd><b>{selected.due}</b><small>Atualizado {selected.updated.toLowerCase()}</small></dd></div>
          <div><dt><LinkSimple/>Dependência real</dt><dd><b>{selected.dependency}</b><small>{selected.status==="Em paralelo"?"Não impede outras frentes de avançarem.":"Monitorada pelo PMO."}</small></dd></div>
          <div><dt><FileText/>Evidência atual</dt><dd><b>{selected.evidence}</b><small>Fonte do percentual apresentado.</small></dd></div>
        </dl>
        <div className="pdm-handoff"><small>PRÓXIMO HANDOFF</small><p>{selected.handoff}</p><ArrowRight/></div>
        <div className="pdm-actions"><button className="ghost" onClick={email}><Envelope/>Cobrar por e-mail</button><button className="ghost" onClick={registerEvidence}><FileText/>Registrar evidência</button><button className="primary" onClick={conclude}><CheckCircle/>Concluir entrega</button></div>
      </aside>
    </div>
  </div>;
}
