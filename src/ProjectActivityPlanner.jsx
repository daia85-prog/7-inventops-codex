...
import {
  CheckCircle, Clock, Envelope, FileText, Funnel, LinkSimple, Plus,
  ShieldCheck, User, Warning, XCircle
...
function createPlan(project){
  ...
  return createProjectDeliveries(project).flatMap((delivery,areaIndex)=>
    taskCatalog[delivery.area].map((name,taskIndex)=>{
      ...
      const status=delivery.status==="Concluída"||firstDone?"Concluída":taskIndex===0?"Em andamento":taskIndex===1&&delivery.progress>=55?"Em andamento":"Não iniciada";
      const dependencyType=["INF","CMP"].includes(delivery.area)&&taskIndex===2?"Externa":delivery.area==="PLC"&&taskIndex===1?"Bloqueante":"Paralela";
      return {
        id:`${project.code}-${delivery.area}-${taskIndex+1}`,
        wbs:`${areaIndex+1}.${taskIndex+1}`,
        area:delivery.area,department:delivery.department,delivery:delivery.delivery,
        name,owner:delivery.owner,email:delivery.email,due:delivery.due,status,
        dependencyType,dependency:taskIndex===0?delivery.dependency:`Conclusão da atividade ${areaIndex+1}.${taskIndex}`,
...
export function ProjectActivityPlanner({project,onUpdate,notify}){
  ...
  const selectFilter=option=>{
    setFilter(option);
    const candidates=plan.filter(item=>(area==="Todas"||item.area===area)&&(
      option==="Todas"||option==="Bloqueantes"&&item.dependencyType==="Bloqueante"||
  ...
  const selectArea=code=>{
    setArea(code);
    const candidate=plan.find(item=>(code==="Todas"||item.area===code)&&(filter==="Todas"||filter==="Bloqueantes"&&item.dependencyType==="Bloqueante"||filter==="Em andamento"&&item.status==="Em andamento"||filter==="Sem evidência"&&item.evidence==="Pendente"||filter==="Concluídas"&&item.status==="Concluída"));
  ...
  const addTask=event=>{
    ...
    const next=[...plan,{
      id:`${project.code}-${draft.area}-${Date.now()}`,wbs:`${deliveries.findIndex(item=>item.area===draft.area)+1}.${count+1}`,
      area:draft.area,department:delivery.department,delivery:delivery.delivery,name:draft.name.trim(),
      owner:delivery.owner,email:delivery.email,due:draft.due,status:"Não iniciada",
      dependencyType:draft.dependencyType,dependency:"Definida na criação da atividade",evidence:"Pendente",
      impact:`Sustenta a entrega “${delivery.delivery}”.`
    }];
    persist(next,`Atividade criada e vinculada a ${delivery.owner}.`);
    setCreating(false);setDraft({name:"",area:"INF",due:"25 jul",dependencyType:"Paralela"});
  };
  const registerEvidence=()=>{
    ...
    persist(plan.map(item=>item.id===selected.id?{...item,evidence:value.trim(),status:item.status==="Não iniciada"?"Em andamento":item.status}:item),"Evidência vinculada à atividade.");
  };
  const conclude=()=>{
    if(selected.evidence==="Pendente"){notify("Registre a evidência antes de concluir a atividade.");return}
  ...
  return <div className="pap">
    <section className="pap-hero"><div><small>PLANO INTEGRADO DO PROJETO</small><h3>Da entrega da área até a atividade executável.</h3><p>As equipes trabalham em paralelo, mas cada tarefa mantém responsável, dependência, prazo e evidência.</p></div><button className="primary" onClick={()=>setCreating(true)}><Plus/>Nova atividade</button></section>
    ...
    <div className="pap-controls">
      <div><Funnel/>{["Todas","Em andamento","Bloqueantes","Sem evidência","Concluídas"].map(option=><button key={option} className={filter===option?"active":""} onClick={()=>selectFilter(option)}>{option}</button>)}</div>
      <label>Área<select value={area} onChange={event=>selectArea(event.target.value)}><option>Todas</option>{deliveries.map(item=><option key={item.area}>{item.area}</option>)}</select></label>
    </div>
    <div className="pap-workspace">
      <article className="pap-list">
        <header><span>WBS / atividade</span><span>Área</span><span>Responsável</span><span>Dependência</span><span>Prazo</span><span>Status</span></header>
        {filtered.map(item=><button key={item.id} className={selected.id===item.id?"selected":""} onClick={()=>setSelectedId(item.id)}>
          <span><small>{item.wbs} · {item.delivery}</small><b>{item.name}</b></span><span><i>{item.area}</i><small>{item.department}</small></span>
          <span><User/><b>{item.owner}</b><small>{item.email}</small></span><span><em className={item.dependencyType.toLowerCase()}>{item.dependencyType}</em><small>{item.dependency}</small></span>
          <span><Clock/><b>{item.due}</b></span><span><strong>{item.status}</strong><small>{item.evidence==="Pendente"?"sem evidência":"com evidência"}</small></span>
        </button>)}
      </article>
      <aside className="pap-detail">
        <header><span>{selected.area}</span><div><small>{selected.wbs} · {selected.department}</small><h3>{selected.name}</h3></div><em>{selected.status}</em></header>
        <section><small>POR QUE ESTA ATIVIDADE IMPORTA</small><p>{selected.impact}</p></section>
        <dl>
          <div><dt><User/>Responsável</dt><dd><b>{selected.owner}</b><small>{selected.email}</small></dd></div>
          <div><dt><LinkSimple/>Dependência</dt><dd><b>{selected.dependencyType}</b><small>{selected.dependency}</small></dd></div>
          <div><dt><FileText/>Evidência</dt><dd><b>{selected.evidence}</b><small>Obrigatória para concluir.</small></dd></div>
        </dl>
        <div><button className="ghost" onClick={prepareEmail}><Envelope/>Preparar e-mail</button><button className="ghost" onClick={registerEvidence}><FileText/>Registrar evidência</button><button className="primary" disabled={selected.status==="Concluída"} onClick={conclude}><CheckCircle/>{selected.status==="Concluída"?"Concluída":"Concluir atividade"}</button></div>
        <footer><ShieldCheck/>Toda alteração atualiza o projeto e preserva a rastreabilidade.</footer>
      </aside>
    </div>
    {creating?<div className="modal-layer" onMouseDown={event=>event.target===event.currentTarget&&setCreating(false)}><form className="pap-create" onSubmit={addTask}><header><div><small>NOVA ATIVIDADE</small><h2>Distribuir trabalho com contexto</h2></div><button type="button" onClick={()=>setCreating(false)} aria-label="Fechar nova atividade"><XCircle/></button></header><label className="wide">Atividade<input autoFocus value={draft.name} onChange={event=>setDraft({...draft,name:event.target.value})} placeholder="Ex.: Validar checklist de conectividade"/></label><label>Área<select value={draft.area} onChange={event=>setDraft({...draft,area:event.target.value})}>{deliveries.map(item=><option key={item.area}>{item.area}</option>)}</select></label><label>Prazo<input value={draft.due} onChange={event=>setDraft({...draft,due:event.target.value})}/></label><label>Tipo de dependência<select value={draft.dependencyType} onChange={event=>setDraft({...draft,dependencyType:event.target.value})}><option>Paralela</option><option>Bloqueante</option><option>Externa</option></select></label><div className="pap-owner-preview"><User/><span><small>RESPONSÁVEL AUTOMÁTICO</small><b>{deliveries.find(item=>item.area===draft.area)?.owner}</b><em>{deliveries.find(item=>item.area===draft.area)?.email}</em></span></div><footer><button type="button" className="ghost" onClick={()=>setCreating(false)}>Cancelar</button><button className="primary"><Plus/>Criar e vincular</button></footer></form></div>:null}
  </div>;
}
