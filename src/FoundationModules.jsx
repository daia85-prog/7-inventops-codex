...
export function ActionCenter({notify}){
  ...
  const registerEvidence=id=>{setActions(current=>current.map(a=>a.id===id?{...a,evidenceState:"Anexada",status:"Em validação"}:a));notify("Evidência registrada. A atividade entrou na fila de validação técnica.")};
  const approve=id=>{setActions(current=>current.map(a=>a.id===id?{...a,status:"Concluído"}:a));notify("Atividade concluída com evidência auditável vinculada ao projeto.")};
  const prepareCommunication=a=>setCommunication(a);
  const message=communication?`InventOps · ${communication.project}\n${communication.task}\nStatus: ${communication.status}\nPrazo: ${communication.due}\nResponsável: ${communication.owner}\nPróxima evidência: ${communication.evidence}`:"";
  return <section className="page foundation-page analyst-workspace">
    <div className="analyst-brief"><div><small>MEU DIA · VISÃO DO ANALISTA</small><h2>Bom dia, Daiana. Três ações protegem os próximos marcos.</h2><p>O InventOps organizou sua fila por impacto no Go Live, SLA e dependências — sem depender de cobranças espalhadas em grupos.</p></div><span><ShieldCheck/><small>GOVERNANÇA</small><b>100% com responsável</b><em>e-mail corporativo vinculado</em></span></div>
    <div className="foundation-metrics"><Metric icon={BellRinging} label="AÇÃO IMEDIATA" value="1 P0" note="SLA em curso" tone="red"/><Metric icon={ClockCountdown} label="VENCEM HOJE" value="2 tarefas" note="1 dependência externa" tone="yellow"/><Metric icon={CheckCircle} label="EM VALIDAÇÃO" value={`${actions.filter(a=>a.status==="Em validação").length}`} note="com evidência anexada" tone="green"/><Metric icon={UsersThree} label="SEM RESPONSÁVEL" value="0" note="governança íntegra"/></div>
    <div className="action-toolbar"><div><h2>Fila inteligente de execução</h2><p>Prioridade, contexto, dono, prazo e evidência em uma única tela.</p></div><div>{["Minha fila","Hoje","Aguardando","P0"].map(x=><button className={filter===x?"active":""} key={x} onClick={()=>setFilter(x)}>{x}</button>)}</div></div>
    <div className="analyst-grid"><div className="action-board">{shown.map(a=><button type="button" key={a.id} className={`action-item ${a.p.toLowerCase()} ${selectedId===a.id?"selected":""}`} onClick={()=>setSelectedId(a.id)}><span className="priority-pill">{a.p}</span><div><small>{a.project} · {a.dept}</small><h3>{a.task}</h3><span><UsersThree/>{a.owner}<CalendarBlank/>{a.due}</span></div><em>{a.status}</em><ArrowRight/></button>)}</div>
      <aside className="task-context"><header><span className={`priority-pill ${selected.p.toLowerCase()}`}>{selected.p}</span><div><small>{selected.project} · {selected.dept}</small><h3>{selected.task}</h3></div></header><div className="context-impact"><Warning/><span><small>POR QUE ISSO IMPORTA</small><p>{selected.impact}</p></span></div><dl><div><dt>Responsável vinculado</dt><dd>{selected.owner}<small>{selected.email}</small></dd></div><div><dt>Dependência</dt><dd>{selected.dependency}<em className={selected.dependencyState.toLowerCase()}>{selected.dependencyState}</em></dd></div><div><dt>Evidência para concluir</dt><dd>{selected.evidence}<small className={selected.evidenceState==="Anexada"?"ready":""}>{selected.evidenceState}</small></dd></div></dl><div className="context-actions"><button className="ghost" onClick={()=>prepareCommunication(selected)}><Envelope/>Preparar atualização</button>{selected.evidenceState!=="Anexada"?<button className="primary" onClick={()=>registerEvidence(selected.id)}><Database/>Registrar evidência</button>:<button className="primary" disabled={selected.status==="Concluído"} onClick={()=>approve(selected.id)}><CheckCircle/>{selected.status==="Concluído"?"Concluída":"Validar e concluir"}</button>}</div><footer><ShieldCheck/>Conclusão bloqueada até existir evidência auditável.</footer></aside>
    </div>
    <Panel title="Execução paralela · NAVEPARK" subtitle="As áreas avançam juntas; apenas dependências reais bloqueiam o trabalho"><div className="parallel-flow">{[["INF","Topologia das VMs","Em andamento","72%"],["CMP","Compra de servidores","Em paralelo","48%"],["ESP","Especificação funcional","Em paralelo","64%"],["DEV","Integrações WCS","Aguardando INF","26%"]].map((x,i)=><div key={x[0]} className={i===3?"blocked":""}><span>{x[0]}</span><section><b>{x[1]}</b><small>{x[2]}</small><i><em style={{width:x[3]}}/></i></section><strong>{x[3]}</strong>{i<3?<ArrowRight/>:null}</div>)}</div><p className="parallel-note"><Sparkle/><span><b>Leitura do InventOps:</b> Compras e Especificação continuam trabalhando. Somente DEV aguarda a topologia de Infra — o sistema não transforma sequência de departamentos em bloqueio artificial.</span></p></Panel>
    {communication?<div className="modal-layer" onMouseDown={e=>e.target===e.currentTarget&&setCommunication(null)}><article className="analyst-communication" role="dialog" aria-modal="true"><header><div><small>ATUALIZAÇÃO OPERACIONAL</small><h2>Mensagem pronta para revisão</h2><p>O destinatário vem da tarefa vinculada ao e-mail corporativo.</p></div><button aria-label="Fechar" onClick={()=>setCommunication(null)}><XCircle/></button></header><div className="communication-recipient"><Envelope/><span><small>DESTINATÁRIO</small><b>{communication.email}</b></span><em>{communication.dept}</em></div><pre>{message}</pre><div><button className="ghost" onClick={()=>{navigator.clipboard?.writeText(message);notify("Atualização copiada e registrada no histórico.")}}><ClipboardText/>Copiar</button><a className="whatsapp" href={`https://wa.me/?text=${encodeURIComponent(message)}`} target="_blank" rel="noreferrer"><WhatsappLogo/>WhatsApp</a><a className="primary" href={`mailto:${communication.email}?subject=${encodeURIComponent(`InventOps · ${communication.project}`)}&body=${encodeURIComponent(message)}`}><Envelope/>Abrir Outlook</a></div><footer><ShieldCheck/>O InventOps prepara e registra; o usuário revisa antes do envio.</footer></article></div>:null}
  </section>;
}
