...
export function AreasPage(){
  ...
  return <section className="page foundation-page"><div className="area-journey"><div><small>JORNADA CROSS-DEPARTMENT Â· TITANO</small><h2>14 Ã¡reas, uma Ãºnica verdade operacional</h2><p>Cada etapa mostra progresso, capacidade e bloqueios sem perder a visÃ£o horizontal do projeto.</p></div><div className="journey-strip">{departments.map((d,i)=><span className={d.load>100?"over":""} key={d.code}><b>{d.code}</b><small>{i<3?"ConcluÃ­do":i<10?"Em curso":"Planejado"}</small></span>)}</div></div><div className="area-toolbar"><div><button className={selected==="Todos"?"active":""} onClick={()=>setSelected("Todos")}>Todas as Ã¡reas</button><button className={selected==="Sobrecarga"?"active":""} onClick={()=>setSelected("Sobrecarga")}>Sobrecarga</button></div><span><i/> acima de 100% de capacidade</span></div><div className="area-grid">{shown.map(d=><article key={d.code} className={d.load>100?"over":""}><header><span>{d.code}</span><div><b>{d.name}</b><small>{d.owner}</small></div><em>{d.load}%</em></header><div><span><small>PROGRESSO</small><b>{d.progress}%</b></span><i><em style={{width:`${d.progress}%`}}/></i></div><footer><span>{d.blocked} bloqueios ativos</span><b>{d.load>100?"Replanejar":"Capacidade controlada"}</b></footer></article>)}</div></section>;
}
...
export function AdminGovernance({role,setRole,notify}){
  ...
  const permissions=["Visualizar","Editar operaÃ§Ã£o","Gerenciar projetos","Administrar acessos","Configurar integraÃ§Ãµes"];
  return <section className="page foundation-page"><div className="admin-banner"><ShieldCheck/><div><small>GOVERNANÃ‡A DE ACESSO</small><h2>PermissÃ£o clara, aÃ§Ã£o auditÃ¡vel</h2><p>A demonstraÃ§Ã£o aplica perfis na interface. Em produÃ§Ã£o, a mesma regra serÃ¡ validada novamente no servidor.</p></div><span><LockKey/>SessÃ£o ativa</span></div><div className="foundation-grid equal"><Panel title="Simular perfil de acesso" subtitle="Use na apresentaÃ§Ã£o para demonstrar o RBAC"><div className="role-selector">{Object.keys(roles).map(r=><button className={role===r?"active":""} key={r} onClick={()=>{setRole(r);notify(`Perfil alterado para ${r}. A navegaÃ§Ã£o foi recalculada.`)}}><UserGear/><span><b>{r}</b><small>{r==="Admin"?"Controle total":r==="Viewer"?"Somente leitura":"Acesso operacional"}</small></span></button>)}</div></Panel><Panel title="Regras obrigatÃ³rias" subtitle="ValidaÃ§Ãµes que protegem a qualidade do dado"><ul className="governance-rules"><li><CheckCircle/><span><b>ConcluÃ­do = 100%</b><small>Sem atraso pendente ou atividade aberta.</small></span></li><li><CheckCircle/><span><b>Bloqueado exige plano</b><small>Categoria, responsÃ¡vel, prÃ³xima aÃ§Ã£o e data.</small></span></li><li><CheckCircle/><span><b>ImportaÃ§Ã£o transacional</b><small>Arquivo invÃ¡lido nÃ£o altera a base.</small></span></li><li><CheckCircle/><span><b>Link seguro do analista</b><small>Token expirÃ¡vel vinculado a e-mail e tarefa.</small></span></li></ul></Panel></div><Panel title="Matriz de permissÃµes" subtitle="Rotas e aÃ§Ãµes por perfil"><div className="permission-table"><header><span>PermissÃ£o</span>{Object.keys(roles).map(r=><span key={r}>{r}</span>)}</header>{permissions.map((p,i)=><div key={p}><b>{p}</b>{Object.keys(roles).map(r=><span key={r}>{roles[r][i]?<CheckCircle/>:<XCircle/>}</span>)}</div>)}</div></Panel><Panel title="Trilha de auditoria" subtitle="Quem fez o quÃª, quando e sobre qual registro"><div className="audit-log">{[["21:38","Douglas","Alterou prazo do Go Live","TITANO"],["20:54","Daiana Costa","Anexou evidÃªncia REV4","QUELUZ"],["19:42","Sistema IoT","Criou alerta P0","TITANO"],["18:17","Ivan","Atualizou bloqueio de VPN","MARKET PERU"]].map(x=><div key={x.join()}><span>{x[0]}</span><b>{x[1]}</b><p>{x[2]}</p><em>{x[3]}</em></div>)}</div></Panel></section>;
}
...
const LOGIN_DEPARTMENTS=["Pós-vendas","Comercial","PM","PCP","Compras","Eng. Mecânica","Eng. Elétrica","Produção","Montagem","Infraestrutura","Espec. Software","WCS Velox","Implantação","PLC"];
const LOGIN_TEXT={
  pt:{
