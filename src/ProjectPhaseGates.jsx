...
function initialGates(project){
  ...
  return definitions.map((phase,phaseIndex)=>({
    id:`${project.code}-G${phaseIndex+1}`,number:phaseIndex+1,name:phase.name,purpose:phase.purpose,
    ...
    decidedAt:phaseIndex+1<project.phase?`${10+phaseIndex} jul · 16:40`:"—",
    criteria:phase.criteria.map(([area,title,expected],criterionIndex)=>({
      id:`${project.code}-G${phaseIndex+1}-C${criterionIndex+1}`,area,title,expected,
...
export function ProjectPhaseGates({project,onUpdate,notify}){
  ...
  const summary=useMemo(()=>({approved:gates.filter(g=>g.status==="Aprovado").length,pending:currentGate?.criteria.filter(c=>c.status!=="Aprovado").length||0}),[gates,currentGate]);
  ...
  const registerEvidence=criterion=>{
    ...
    persist(gates.map(gate=>gate.id===selected.id?{...gate,criteria:gate.criteria.map(item=>item.id===criterion.id?{...item,evidence:value,status:"Em validação"}:item)}:gate),"Evidência registrada e enviada para validação.");
  };
  const approveCriterion=criterion=>{
    if(criterion.evidence==="Pendente"){notify("Registre a evidência antes de aprovar o critério.");return}
  ...
  const approveGate=()=>{
    if(selected.number!==project.phase){notify("Somente o gate da fase atual pode ser aprovado.");return}
    if(selected.criteria.some(item=>item.status!=="Aprovado")){notify("O gate permanece bloqueado: existem critérios sem aprovação.");return}
  ...
  return <div className="ppg">
    <section className="ppg-hero"><div><small>GATES DE GOVERNANÇA</small><h3>Fase só avança quando o critério de saída está provado.</h3><p>Percentual não aprova gate. A decisão exige evidência, responsável e registro do PMO.</p></div><span><ShieldCheck/><small>FASE ATUAL</small><b>{project.phase}/7 · {definitions[project.phase-1].name}</b><em>{summary.pending} critérios pendentes</em></span></section>
    ...
    <div className="ppg-workspace">
      ...
      <aside className="ppg-decision">
        ...
        {selected.status==="Aprovado"?<div className="ppg-approved"><CheckCircle weight="fill"/><p><b>Decisão registrada</b><span>{selected.decidedBy}</span><small>{selected.decidedAt}</small></p></div>:<div className="ppg-block"><Warning/><p><b>{selected.criteria.length-approved} critérios impedem o avanço</b><span>O projeto continua executando atividades paralelas, mas não muda de fase.</span></p></div>}
